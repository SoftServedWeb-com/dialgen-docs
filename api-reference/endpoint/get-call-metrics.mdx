...
<Info>
  All endpoints should pass in the `userId` query parameter. You can find your `userId` in the [sa.dialgen.ai](https://sa.dialgen.ai) dashboard.
</Info>
...
*   **Example Response:**
    ...
    *   **`metricSchema`**: The schema definition for `metricJson`.
    ...
    <Note>
      The structure of `metricSchema` depends on the **Custom Metrics** configured in the Dialgen dashboard. You can define and view these values by navigating to **Agent > Settings > Metrics**.
    </Note>
