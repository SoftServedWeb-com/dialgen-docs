...
<Info>
  All endpoints should pass in the `userId` in the request body. You can find your `userId` in the [sa.dialgen.ai](https://sa.dialgen.ai) dashboard.
</Info>
...
*   **`agentId`** (string, required): The agent ID to use for the entire batch. See the [Get Agent](/api-reference/endpoint/get-agent) endpoint for more details.
...
*   **`options`** (object, optional): Batch processing configuration. All fields within this object are optional and will use default values if not provided.
    ...
    *   **`webhooks`** (object, optional): Webhook URLs for event notifications.
        ...
        *   **`onCallComplete`** (string, optional): Called after each individual call completes. The payload sent to this URL matches the response format of the [Get Call Metrics](/api-reference/endpoint/get-call-metrics) endpoint.
