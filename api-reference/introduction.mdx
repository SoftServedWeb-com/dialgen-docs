...
<Info>
  **Important Note on `userId`**
  ...
  All endpoints implicitly require `userId` which is derived from your API Key. While some endpoints may accept it as an optional parameter for verification, the system primarily uses the authenticated user's ID tied to the provided Bearer Token.
</Info>
